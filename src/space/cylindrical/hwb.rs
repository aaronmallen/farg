...
#[cfg(feature = "serde")]
impl<'de, S> serde::Deserialize<'de> for Hwb<S>
where
  S: RgbSpec,
{
  fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
    ...
    let data = HwbData::deserialize(deserializer)?;
    Ok(Self {
      h: data.h,
      w: data.w,
      b: data.b,
...
impl<OS, S> From<super::Hsi<OS>> for Hwb<S>
where
  OS: RgbSpec,
  S: RgbSpec,
{
...
#[cfg(feature = "space-okhwb")]
impl<S> From<Okhwb> for Hwb<S>
where
  S: RgbSpec,
{
  fn from(okhwb: Okhwb) -> Self {
    okhwb.to_rgb::<S>().to_hwb()
  }
}
...
#[cfg(feature = "serde")]
impl<S> serde::Serialize for Hwb<S>
where
  S: RgbSpec,
{
  fn serialize<Ser: serde::Serializer>(&self, serializer: Ser) -> Result<Ser::Ok, Ser::Error> {
    ...
    let mut state = serializer.serialize_struct("Hwb", field_count)?;
