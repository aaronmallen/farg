...
#[cfg(feature = "serde")]
impl<'de> serde::Deserialize<'de> for Okhsl {
  fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
    ...
    let data = OkhslData::deserialize(deserializer)?;
    Ok(Self {
      h: data.h,
      s: data.s,
      l: data.l,
...
#[cfg(feature = "serde")]
impl serde::Serialize for Okhsl {
  fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
    use serde::ser::SerializeStruct;

    let field_count = if self.alpha.0 < 1.0 { 4 } else { 3 };
    let mut state = serializer.serialize_struct("Okhsl", field_count)?;
