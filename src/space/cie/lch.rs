...
#[cfg(feature = "serde")]
impl<'de> serde::Deserialize<'de> for Lch {
  fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
    ...
    let data = LchData::deserialize(deserializer)?;
    Ok(Self {
      l: data.l,
      c: data.c,
      h: data.h,
...
#[cfg(feature = "serde")]
impl serde::Serialize for Lch {
  fn serialize<S: serde::Serializer>(&self, serializer: S) -> Result<S::Ok, S::Error> {
    use serde::ser::SerializeStruct;

    let field_count = if self.alpha.0 < 1.0 { 4 } else { 3 };
    let mut state = serializer.serialize_struct("Lch", field_count)?;
