...
Security is important for a color science library that may be used in production applications processing user input.
...
- Allow denial of service attacks through resource exhaustion (e.g., crafted inputs causing excessive computation)
- Enable unexpected behavior through numeric overflow or underflow in color conversions
- Expose sensitive information in error messages or logs
- Allow unsafe memory access through incorrect `unsafe` code usage
...
- Color conversion accuracy differences that don't affect security
...
When contributing to Farg, please follow these security guidelines:

- Never commit sensitive data (API keys, tokens, passwords) to the repository
- Validate and sanitize all user-facing inputs, especially when parsing color strings
- Avoid `unsafe` code unless absolutely necessary, and document safety invariants
- Handle numeric edge cases (NaN, infinity, overflow) gracefully in color conversions
...
[vulnerability-report]: https://github.com/aaronmallen/farg/security/advisories/new
